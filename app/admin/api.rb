ActiveAdmin.register Api do
  ...
  form do |f|
    f.inputs do
      ...
      f.input :request_type, as: :select, label: "Request Type (get, post, put, delete etc.)", collection: ['GET', 'POST', 'PUT', 'PATCH', 'DELETE'], include_blank: false
      f.input :required_params, as: :string, label: "Required params (eg: platform, version) - comma separated"
      f.input :required_headers, as: :string, label: "Required headers (eg: auth-token) - comma separated (Work in Progress, not functional)"
